...
class VKClient:
    ...
    def choose_fotos_max_size(self):
        ...
        for fotos in tqdm(photo_all_size['response']['items']):
            key = f' {fotos['likes']['count']}_{str(fotos['id'])[-2:]}.jpg'
            for foto in fotos['sizes']:
                if foto['type'] == 'w':
                    photos_max_size[key] = [foto['url'], foto['type']]

        for k, v in tqdm(photos_max_size.items()):
            photo_info_dict = {}
...
class YDClient:
    ...
    # записываем фотографии на диск в созданную папку
    ...
    def yd_write_fotos(self):
        ...
        for key, value in photos.items():
            ...
            params = {"path": f'{self.path}/{file_name}'}
            headers = {'Authorization': auto}
